...
const TOGGLE_DEFS = [
  ...
  { key: "fastroute", name: "Программный fastpath: выключать без аппаратного NAT",
    desc: "На платах без аппаратного NAT (прошивка Keenetic, портированная на Cudy и подобные) поток после рукопожатия уводит программный маршрутный кэш прошивки: роутер видит первый ClientHello, но не видит повторов и сбросов, поэтому подбор стратегий стоит на первой и «не работает ничего, кроме ютуба». Опция выключает этот кэш только там, где железного ускорителя нет; на обычных Keenetic она ничего не меняет. Выключите, если хотите оставить кэш ради экономии процессора." },
