...
generate_nfqws2_opt_from_strategies() {
    ...
    #
    #   retrans=3   ВРЕМЕННО ВЕРНУТО С 2 (14.09.2026, p-84.12).
    #
    #               Порог 2 безопасен только на ядре, где вердикт привязан к
    #               поколению стратегии: там чужой запоздалый повтор просто не
    #               засчитывается. Такое ядро едет архивом форка и приезжает
    #               ТОЛЬКО полной переустановкой, а p-84.11 из-за дефекта
    #               апдейтера уехал патчем — то есть на флоте оказались новые
    #               детекторы, сниженный порог и СТАРОЕ ядро. В этой связке
    #               порог 2 даёт больше ложных ротаций, а не меньше: обычная
    #               потеря двух сегментов уже завершает попытку.
    #
    #               Вернуть 2 следующим выпуском — тем, который действительно
    #               доставит ядро (Z2K_RELEASE_FULL_INSTALL=1 на исправленном
    #               апдейтере). Пинится tests/test_profile_observation.
