...
apply_category_strategies_v2() {
    ...
    # ЧЕЛОВЕК ОСТАНОВИЛ СЕРВИС — ЗНАЧИТ ОН ОСТАНОВЛЕН, А НЕ СЛОМАН.
    #
    # ENABLED=0 ставит сам человек кнопкой в панели или пунктом меню, и мы этот
    # флаг бережём через переустановку намеренно (иначе остановленный сервис
    # воскресал бы каждой ночной обновой). Но проверка ниже спрашивала только
    # «бежит ли nfqws2», не спрашивая, должен ли он бежать: init при ENABLED=0
    # честно отвечает «zapret2 is disabled in config» и не стартует, проверка
    # засчитывала это за провал, apply возвращал 1 — и установщик откатывал
    # ВСЮ переустановку на прежнее дерево.
    #
    # Снаружи это выглядело так (жалоба из поля 16.09.2026): останавливаешь
    # сервис, запускаешь переустановку из меню — она никогда не завершается
    # удачей; запускаешь сервис перед переустановкой — всё проходит. Причина не
    # в переустановке, а в том, что мы считали намеренную остановку поломкой.
    local _svc_enabled
    _svc_enabled=$(safe_config_read "ENABLED" "$zapret_config" "1")
    if [ "$_svc_enabled" = "0" ]; then
        print_info "Сервис остановлен вами — оставляю остановленным"
        print_info "Включить: пункт меню «Управление сервисом» или кнопка в веб-панели"
        return 0
    fi
